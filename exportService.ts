  ...
  const children: any[] = [
    ...
    ...(plan.tien_trinh_day_hoc || []).flatMap(hd => {
      const hdChildren = [
        ...
        // Detailed 4 Steps with GV/HS distinctions
        ...
        ...(hd.to_chuc_thuc_hien?.chuyen_giao_nhiem_vu || []).map(s => {
          ...
          return new Paragraph({ children: [new TextRun({ text: s, bold: isGV, size: 24, font: "Times New Roman" })], indent: { left: 1080 } });
        }),
        ...
        ...(hd.to_chuc_thuc_hien?.thuc_hien_nhiem_vu || []).map(s => {
          ...
          return new Paragraph({ children: [new TextRun({ text: s, italics: isHS, size: 24, font: "Times New Roman" })], indent: { left: 1080 } });
        }),
        ...
        ...(hd.to_chuc_thuc_hien?.ket_luan_nhan_dinh || []).map(s => new Paragraph({ children: [new TextRun({ text: s, bold: true, size: 24, font: "Times New Roman" })], indent: { left: 1080 } }))
      ];
      return hdChildren;
    }),
