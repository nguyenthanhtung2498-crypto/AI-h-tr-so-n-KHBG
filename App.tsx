...
const App: React.FC = () => {
  // ✅ session: load từ localStorage chỉ khi chạy trên browser
  const [session, setSession] = useState<UserSession | null>(null);

  // ✅ API key: chỉ lưu RAM, không lưu localStorage
  ...
  // Khi session load xong thì đồng bộ giao_vien
  useEffect(() => {
    if (session?.username) {
      setInputs(prev => ({ ...prev, giao_vien: session.username }));
    }
  }, [session?.username]);
  ...
  // ✅ Login nhận thêm apiKey (từ LoginPage)
  const handleLogin = (userData: UserSession, apiKey: string) => {
    ...
    setSession(userData);
    setUserApiKey(apiKey); // ✅ chỉ lưu RAM
  ...
  const handleFileUpload = async (
    ...
    // ✅ Nếu chưa có API key thì chặn luôn
    if (!userApiKey.trim()) {
      setError('Bạn chưa nhập API key Gemini. Vui lòng đăng xuất và nhập lại API key.');
      return;
    }
    ...
    try {
      ...
      if (fieldName === ('textbookPdfData' as any)) {
        ...
        setInputs(prev => ({ ...prev, textbookPdfData: base64 } as any));
        ...
        try {
          ...
          setError(err?.message || 'Lỗi phân tích PDF.');
        } finally {
      ...
      // Auto extract metadata từ giáo án mẫu
      ...
      setError(`Lỗi tệp: ${err?.message || err}`);
    }
  };
        ...
        <div className="max-w-2xl mx-auto py-12 px-6 bg-white/95 backdrop-blur-xl rounded-[3rem] shadow-2xl border border-indigo-100 animate-in fade-in zoom-in duration-500">
          ...
          <div className="space-y-2 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar">
            {catalog.map((lesson, idx) => (
              <button
                key={idx}
                onClick={() => selectLessonFromCatalog(lesson)}
                className="w-full text-left p-4 bg-white border border-gray-100 rounded-2xl hover:bg-indigo-50 hover:border-indigo-200 transition-all flex items-center group shadow-sm"
              >
                <span className="w-8 h-8 rounded-lg bg-gray-50 text-gray-400 flex items-center justify-center text-xs font-bold mr-4 group-hover:bg-indigo-600 group-hover:text-white transition-colors">
                  {idx + 1}
                </span>
  ...
  const getAutoFilledClass = (name: string) =>
    autoFilledFields.has(name)
      ? 'border-indigo-400 ring-4 ring-indigo-50 bg-indigo-50/20 transition-all duration-700'
      ...
      {view === 'RESULT' && result ? (
        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-700">
          <div className="flex justify-between items-center mb-4 no-print">
            <button
              onClick={() => setView('FORM')}
              className="flex items-center text-indigo-600 font-black text-[10px] uppercase tracking-widest hover:translate-x-[-4px] transition-transform"
            >
            ...
            <div className="flex space-x-3">
              <button
                ...
                className="bg-white border px-6 py-3 rounded-2xl hover:bg-gray-50 flex items-center shadow-sm font-black text-[10px] uppercase tracking-widest transition-all"
              >
                Xuất PDF / In
              </button>
              <button
                onClick={() => exportToWord(result)}
                className="bg-blue-600 text-white px-6 py-3 rounded-2xl hover:bg-blue-700 shadow-xl font-black text-[10px] uppercase tracking-widest transition-all"
              >
                Lưu Word
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-wrap gap-4 items-end">
                <div className="flex-1 min-w-[200px]">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border-gray-100 p-2.5 border focus:ring-2 focus:ring-indigo-500/20 text-xs font-bold text-gray-600"
                  />
                </div>
                ...
                <div className="w-32">
                  ...
                  <input
                    ...
                    value={inputs.to}
                    onChange={handleInputChange}
                    className="w-full rounded-xl border-gray-100 p-2.5 border text-xs"
                    placeholder="Tổ..."
                  />
                </div>
                ...
                <div className="w-40">
                  ...
                  <input
                    type="text"
                    name="giao_vien"
                    value={inputs.giao_vien}
                    onChange={handleInputChange}
                    className="w-full rounded-xl border-gray-100 p-2.5 border text-xs font-bold text-indigo-700"
                  />
                </div>
              </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-2">
                      ...
                      {autoFilledFields.has('ten_bai_day') && (
                        <span className="bg-indigo-600 text-white text-[8px] font-black px-2 py-0.5 rounded-full uppercase animate-pulse">
                          ✨ AI đã tự điền
                        </span>
                      )}
                    </div>
                    ...
                    <textarea
                      ...
                      onChange={handleInputChange}
                      rows={2}
                      className={`w-full rounded-2xl p-4 border font-black text-xl text-gray-800 shadow-sm resize-none focus:ring-0 ${getAutoFilledClass(
                        'ten_bai_day'
                      )}`}
                  ...
                  <div className="grid grid-cols-3 gap-6">
                    <div className="col-span-1">
                      ...
                      <select
                        ...
                        onChange={handleInputChange}
                        className={`w-full rounded-xl p-3 border text-sm font-bold shadow-sm appearance-none cursor-pointer ${getAutoFilledClass(
                          'subject'
                        )}`}
                      >
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          ...
                          value={inputs.lop}
                          onChange={handleInputChange}
                          className={`w-full rounded-xl p-3 border text-center text-sm font-black shadow-sm ${getAutoFilledClass('lop')}`}
                        />
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <input
                          type="text"
                          name="so_tiet"
                          value={inputs.so_tiet}
                          onChange={handleInputChange}
                          className={`w-full rounded-xl p-3 border text-center text-sm font-black shadow-sm ${getAutoFilledClass(
                            'so_tiet'
                          )}`}
                        />
                    ...
                    <div className="flex items-center justify-between group">
                      ...
                      <div className="relative">
                        <input
                          type="file"
                          accept=".docx,.txt"
                          onChange={(e) => handleFileUpload(e, 'khbg_mau')}
                    ...
                    <div className="flex items-center justify-between group">
                      ...
                      <div className="relative">
                        <input
                          ...
                          onChange={(e) => handleFileUpload(e, 'textbookPdfData' as any)}
                          className="absolute inset-0 opacity-0 cursor-pointer z-10"
                        />
                ...
                <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <label className="flex items-center p-3 bg-gray-50 rounded-2xl cursor-pointer hover:bg-emerald-50 transition-colors">
                      <input
                        type="checkbox"
                        name="integrate_environment"
                        checked={inputs.integrate_environment}
                        onChange={handleInputChange}
                    ...
                    {tichHopNLS && (
                      <div className="px-2">
                        <input
                          type="file"
                          accept=".docx,.txt"
                          onChange={(e) => handleFileUpload(e, 'nang_luc_so')}
          ...
          {/* Cột phải giữ nguyên layout / preview */}
          <div className="lg:col-span-5 sticky top-10">
            <div className="bg-white rounded-[3rem] border-2 border-dashed border-gray-100 p-10 text-center shadow-inner h-[600px] flex flex-col items-center justify-center relative group">
              ...
              <div className="mt-8 flex flex-wrap justify-center gap-2">
                <span className="bg-gray-50 px-3 py-1 rounded-full text-[9px] font-bold text-gray-400 uppercase tracking-widest border border-gray-100">
                  Chuẩn 5512
                </span>
                <span className="bg-gray-50 px-3 py-1 rounded-full text-[9px] font-bold text-gray-400 uppercase tracking-widest border border-gray-100">
                  Tích hợp ANQP
