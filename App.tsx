...
const App: React.FC = () => {
    ...
    try {
      ...
      if ((fieldName === 'khbg_mau' || fieldName === 'phu_luc_1' || fieldName === 'phu_luc_3') && text) {
        ...
        try {
          ...
          setInputs(prev => {
            const next = { ...prev };
            if (meta.ten_bai_day && (!prev.ten_bai_day || prev.ten_bai_day === '')) { 
            ...
            if (meta.lop && (!prev.lop || prev.lop === '6')) { 
              next.lop = meta.lop; 
              filled.add('lop'); 
            }
            if (meta.so_tiet && prev.so_tiet === '1') { 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!hasKey) {
      setError("Vui lòng kích hoạt API Key cá nhân để hệ thống hoạt động ổn định.");
    ...
    try {
      ...
      if (err.message?.includes("entity was not found") || err.message?.includes("API key")) {
        setError("API Key không hợp lệ hoặc đã hết hạn. Hãy chọn lại mã API Key.");
        await window.aistudio?.openSelectKey();
      } else {
        setError("Hệ thống AI gặp sự cố. Vui lòng thử lại sau hoặc kiểm tra API Key.");
      }
    } finally {
      setIsLoading(false);
    }
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
              <div className="flex justify-between items-center mb-3 border-b pb-1">
                ...
                {isExtractingMetadata && (
                  <span className="text-[8px] font-black text-indigo-500 animate-pulse uppercase tracking-widest">🤖 Đang phân tích tệp dữ liệu...</span>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between items-center mb-2">
                    <label className="text-sm font-black text-gray-700 uppercase tracking-tight">Tên bài học cụ thể</label>
                    {autoFilledFields.has('ten_bai_day') && (
                      <span className="text-[7px] font-black bg-emerald-500 text-white px-1.5 py-0.5 rounded-full uppercase">✨ AI TRÍCH XUẤT</span>
                    )}
                  </div>
                  <textarea 
                    name="ten_bai_day" 
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div>
                    <label className="block text-[8px] font-black text-gray-400 uppercase mb-1">Thời lượng (Tiết)</label>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <div className="flex bg-gray-50/50 border-b">
                <button type="button" onClick={() => setInputs(prev => ({ ...prev, autoComposeMode: 'TEMPLATE' }))} className={`flex-1 py-3 text-[9px] font-black uppercase tracking-wider ${inputs.autoComposeMode === 'TEMPLATE' ? 'bg-white text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-400'}`}>📂 Soạn từ giáo án mẫu</button>
              ...
              <div className="p-4 flex items-center justify-between gap-4">
                <div className="flex-1">
                  ...
                  <p className="text-[9px] text-gray-400 italic">Hệ thống sẽ dựa vào tệp này để thiết kế bài dạy.</p>
                </div>
                  ...
                  {inputs.autoComposeMode === 'TEXTBOOK' ? (
                    fileNames.textbookPdfData ? (
                      <div className="w-full h-full bg-emerald-50 border border-emerald-100 rounded-lg flex flex-col items-center justify-center p-2 shadow-inner">
                        ...
                        <button type="button" onClick={() => clearFile('textbookPdfData')} className="text-[7px] font-black text-red-500 uppercase mt-1">Hủy tệp</button>
                      </div>
                    ) : (
                    ...
                    fileNames.khbg_mau ? (
                      <div className="w-full h-full bg-indigo-50 border border-indigo-100 rounded-lg flex flex-col items-center justify-center p-2 shadow-inner">
                        ...
                        <button type="button" onClick={() => clearFile('khbg_mau')} className="text-[7px] font-black text-red-500 uppercase mt-1">Hủy tệp</button>
                      </div>
                    ) : (
                      <div className="relative h-full">
                        ...
                        <div className="w-full h-full rounded-lg border border-dashed border-gray-200 bg-gray-50 flex items-center justify-center text-[9px] font-black text-gray-400 uppercase tracking-widest">Tải tệp (.docx)</div>
                      </div>
                    )
                  )}
                </div>
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
              <h3 className="text-[10px] font-black text-indigo-950 uppercase tracking-widest mb-3 border-b pb-1 flex items-center">
                <span className="w-1.5 h-1.5 bg-indigo-600 rounded-full mr-2"></span>
                IV. Hồ sơ Phụ lục (Cá nhân/Tổ)
              </h3>
              <div className="grid grid-cols-2 gap-3">
                <div className="p-3 bg-gray-50/50 rounded-lg border border-gray-100 flex flex-col">
                  <span className="text-[8px] font-black text-gray-500 uppercase mb-2">Phụ lục 1 (Tổ)</span>
                  {fileNames.phu_luc_1 ? (
                ...
                <div className="p-3 bg-gray-50/50 rounded-lg border border-gray-100 flex flex-col">
                  <span className="text-[8px] font-black text-gray-500 uppercase mb-2">Phụ lục 3 (Cá nhân)</span>
                  {fileNames.phu_luc_3 ? (
                    <div className="flex items-center justify-between">
            ...
            {/* V. TÍCH HỢP & NĂNG LỰC SỐ */}
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
              <h3 className="text-[10px] font-black text-indigo-950 uppercase tracking-widest mb-3 border-b pb-1 flex items-center">
                <span className="w-1.5 h-1.5 bg-indigo-600 rounded-full mr-2"></span>
                V. Tiện ích & Năng lực số (3456)
              </h3>
              <div className="grid grid-cols-4 gap-2 mb-3">
                ...
                <button type="button" onClick={() => setInputs(prev => ({ ...prev, integrate_environment: !prev.integrate_environment }))} className={`p-2 rounded-lg border flex items-center justify-center space-x-2 transition-all ${inputs.integrate_environment ? 'bg-emerald-50 border-emerald-400 text-emerald-700 shadow-sm' : 'bg-white border-gray-100 text-gray-300'}`}>
                  <span className="text-[9px] font-black uppercase">MT</span>
                </button>
                ...
                <button type="button" onClick={() => setInputs(prev => ({ ...prev, nang_luc_so: prev.nang_luc_so ? '' : 'ACTIVE' }))} className={`p-2 rounded-lg border flex items-center justify-center space-x-2 transition-all ${inputs.nang_luc_so ? 'bg-blue-600 border-blue-700 text-white shadow-lg' : 'bg-white border-gray-100 text-gray-300'}`}>
                  <span className="text-[9px] font-black uppercase">NLS</span>
                </button>
              </div>
              ...
              {inputs.nang_luc_so && (
                <div className="mt-2 p-3 bg-blue-50/50 rounded-lg border-2 border-dashed border-blue-300 animate-in slide-in-from-top-2 duration-300">
                  <span className="text-[8px] font-black text-blue-700 uppercase block mb-2">Tài liệu Khung Năng lực số (3456)</span>
                  {fileNames.nang_luc_so ? (
                    <div className="flex items-center justify-between bg-white p-2 rounded-lg border border-blue-200">
                      ...
                      <button type="button" onClick={() => clearFile('nang_luc_so')} className="text-[8px] font-black text-red-500 uppercase">Xóa</button>
                    </div>
                  ) : (
                    <div className="relative group">
                      ...
                      <div className="py-2.5 border-2 border-dashed border-blue-200 rounded-lg text-center text-[8px] font-black text-blue-400 uppercase bg-white group-hover:bg-blue-50 transition-colors">Tải tài liệu NLS (.docx)</div>
                    </div>
                  )}
                </div>
              )}
            ...
            {error && (
              <div className="p-4 bg-red-50 border border-red-200 rounded-xl text-red-600 text-xs font-black animate-pulse text-center uppercase tracking-widest shadow-inner">
                {error}
              </div>
            )}
            ...
            <div className="bg-white rounded-xl shadow-lg p-3">
               <button type="submit" disabled={isLoading || isAnalyzingPdf} className="w-full py-5 bg-[#0f172a] text-white rounded-lg font-black uppercase text-xl tracking-widest shadow-xl hover:bg-black transition-all border-b-4 border-indigo-950 active:border-b-0 active:translate-y-0.5 disabled:opacity-50">
                {isLoading ? "🤖 AI ĐANG THỰC HIỆN SOẠN THẢO..." : "KHỞI TẠO KHBG BẰNG AI"}
               </button>
            </div>
          </form>
        </div>
          ...
          <div ref={resultRef} className="space-y-4 animate-in slide-in-from-bottom-4 duration-700 pb-10 pt-4 border-t-2 border-dashed border-gray-200">
            <div className="flex justify-between items-center no-print px-1">
              <h4 className="text-sm font-black text-indigo-600 uppercase tracking-widest flex items-center">
                ...
                Hệ thống AI đã hoàn thành thiết kế
              </h4>
              <div className="flex space-x-2">
                <button onClick={() => window.print()} className="bg-white border border-gray-200 px-3 py-1.5 rounded-lg font-black text-[9px] uppercase tracking-wider">In ấn</button>
                <button onClick={() => exportToWord(result)} className="bg-indigo-600 text-white px-4 py-1.5 rounded-lg shadow-md font-black text-[9px] uppercase tracking-wider border-b-2 border-indigo-900 active:border-b-0">Xuất file Word</button>
              </div>
            </div>
